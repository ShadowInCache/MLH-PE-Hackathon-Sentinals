...
This section defines safe operational steps for canary rollout verification and rollback.

### Pre-Rollback Checklist

1. Confirm alert context in Grafana (`HighErrorRate`, `CanaryFailureDetected`, `ServiceDown`).
2. Confirm canary pressure and failure symptoms:
   - `sum by (app_version) (rate(url_redirects_total[5m]))`
   - `increase(ghostlink_canary_failures_total[5m])`
3. Confirm release metadata in `/health` includes expected `app_version` and `git_sha`.

### Rollback Plan Preview (No Changes)

Use dry-run targets to print exactly which compose and rollback-state values would change.
...
./scripts/rollback.sh --dry-run app2 v1
```
...
2. Confirm release header and metadata changed as expected:
   - response header `X-GhostLink-Version`
   - `/health` fields under `release`
3. Confirm rollback state metrics are updated:
   - `ghostlink_rollbacks_total`
   - `ghostlink_recovery_attempts_total`
   - `ghostlink_recovery_success_total`
4. Confirm alerts are recovering:
   - `mean_time_to_recover_minutes`
   - `recovery_success_rate`

### Rollback Audit Evidence

Rollback events and state are persisted here:

- `security/rollback_events.log`
- `security/rollback_state.env`

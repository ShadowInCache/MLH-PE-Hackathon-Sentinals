...
if [[ -z "${SHORT_CODE}" ]]; then
  echo "Usage: ./scripts/unquarantine_code.sh <short_code>" >&2
  exit 1
fi
...
TMP_FILE="${BLOCKED_FILE}.tmp"

grep -Fvx "${RULE_LINE}" "${BLOCKED_FILE}" > "${TMP_FILE}" || true
mv "${TMP_FILE}" "${BLOCKED_FILE}"

TIMESTAMP="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
printf "%s action=unquarantine short_code=%s actor=%s\n" "${TIMESTAMP}" "${SHORT_CODE}" "${USER:-unknown}" >> "${AUDIT_LOG}"
...
echo "Unquarantined short code: ${SHORT_CODE}"
